    ...
    def render_passing_walls(self, screen, player):
        ...
        for y in range(y1, y2):
            for x in range(x1, x2):
                ...
                if image:
                    screen.blit(image, (self.x + x * self.tile_size, self.y + y * self.tile_size))
                    print(self.map.get_tile_gid(x, y, 2))
                    if not (406 >= self.map.get_tile_gid(x, y, 2) >= 404) or self.y + (
                            y + 1) * self.tile_size >= player.rect.y + player.rect.height >= self.y + y * self.tile_size and \
                            (
                                    player.rect.x + player.rect.width >= self.x + x * self.tile_size >= player.rect.x or player.rect.x + player.rect.width >= self.x + (
                                    x + 1) * self.tile_size >= player.rect.x):  # сложно объяснить: делает, можно сказать, 3D объекты
                        redrawing = False
        if redrawing:
            player.draw()
